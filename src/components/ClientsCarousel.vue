...
.client-card {
  ...
  /* IMPORTANT: clip SVG strokes / scaled logos */
  overflow: hidden;
...
/* -------------------------
   CLIENT CARD – BOTTOM ROW
   (same sizing for now; keep as separate class)
   ------------------------- */
.client-card--small {
  flex: 0 0 auto;
  display: flex;
...
@media (min Max) { /* <-- remove this line if copied; it's invalid */ }
@media (min-width: 768px) {
  .client-card--small {
    width: 13rem;
    height: 7rem;
...
/* -------------------------
   LOGO INSIDE CARDS
   ------------------------- */
/* Make the <a> fill the card so the img can be centered correctly */
.client-card a,
.client-card--small a {
  width: 100%;
  height: 100%;
...
/* The img scales relative to the card, so media queries affect the card only */
.client-card img,
...
/* Optional hover effect (safe because card clips overflow) */
.client-card:hover img,
.client-card--small:hover img {
  transform: scale(1.05);
}

/* Force LTR inside rows (your original) */
.clients-row,
.clients-marquee,
.clients-marquee__inner {
  direction: ltr;
